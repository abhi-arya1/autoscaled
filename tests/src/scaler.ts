...
export class Autoscaler<Env> extends DurableObject<Env> {
    ...
    private async getAllInstances(): Promise<InstanceRecord[]> {
        const cursor = this.ctx.storage.sql.exec<InstanceRecord>(
            `SELECT * FROM instances ORDER BY pending_requests ASC, last_heartbeat DESC`
        );
    ...
    private async getMetrics(): Promise<{
        ...
        return {
            ...
            healthy_instances: instances.filter((i) => i.healthy === 1).length,
            pending_requests: instances.reduce((sum, i) => sum + i.pending_requests, 0),
            current_cpu: instances.reduce((sum, i) => sum + i.current_cpu, 0),
            current_memory_MiB: instances.reduce((sum, i) => sum + i.current_memory_MiB, 0),
            current_disk_GB: instances.reduce((sum, i) => sum + i.current_disk_GB, 0),
            instances: instances.map((instance) => ({
                name: instance.name,
                created_at: instance.created_at,
                pending_requests: instance.pending_requests,
                current_cpu: instance.current_cpu,
                current_memory_MiB: instance.current_memory_MiB,
                current_disk_GB: instance.current_disk_GB,
                healthy: instance.healthy === 1,
    ...
    override async fetch(request: Request): Promise<Response> {
        ...
        // Check if this is a metrics request (handle with or without trailing slash)
        if (pathname.endsWith("/_internal_metrics") || pathname.endsWith("/_internal_metrics/")) {
