...
export class Autoscaler<Env> extends DurableObject<Env> {
        ...
        if (result.length === 0 || !result[0]) {
            try {
                ...
                if (fallbackResult.length > 0 && fallbackResult[0]) {
                    const binding = this.containerBinding.getByName(
    ...
    override async fetch(request: Request): Promise<Response> {
        try {
            ...
            try {
                ...
                this.ctx.waitUntil(
                    this.decrementPendingRequests(containerName).catch(
                        (err) => {
                            console.error(
                                "Error decrementing pending requests:",
                                err,
                            );
                        },
                    ),
    ...
    private async updateStatus(
        ...
        const healthy =
            state.status === "running" || state.status === "healthy" ? 1 : 0;
